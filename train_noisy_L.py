    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in enumerate(loader_train, 0):
            ...
            if opt.net == "HN":
                ...
                loss = (1.0 * criterion(out_train, img_train) / imgn_train.size()[0] * 2) + (0.024 * criterion(feature_out, feature_img) / (feature_img.size()[0] / 2)) + (1.0 * criterion(out_dn, imgn_train_mid) / imgn_train.size()[0] * 2) + (
                               1.0 * criterion(out_wm, img_train) / imgn_train.size()[0] * 2) + (0.024 * criterion(feature_out_wm, feature_img) / (feature_img.size()[0] / 2))
            else:
                loss = (1.0 * criterion(out_train, img_train) / imgn_train.size()[
                    0] * 2) + (0.024 * criterion(feature_out, feature_img) / (feature_img.size()[0] / 2))
            loss.backward()
